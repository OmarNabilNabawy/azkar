...
class SharedPreference {
  ...
  Future<void> saveData() async {
    ...
    // save zakrMainList
    final jsonZakrList = zakrMainList.map((zakr) => zakr.toJson()).toList();
    await MyApp.sp.setString('zakrModelList', json.encode(jsonZakrList));
  ...
  loadData() {
    ...
    final jsonZakr = MyApp.sp.getString('zakrModelList');
    if (jsonZakr != null) {
      final jsonList = json.decode(jsonZakr) as List<dynamic>;
      zakrMainList = jsonList.map((json) => ZakrModel.fromJson(json)).toList();
    }
  ...
  void loadingDataRequiredNavigatorKeyContext() {
    ...
    favorite.favoriteListView =
        zakrMainList.where((zakr) => zakr.isFavor).toList();
